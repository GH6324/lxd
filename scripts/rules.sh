...
detect_firewall_backend || exit 1
blocked_ports=(3389 8888 54321 65432)
if [ "$FW_BACKEND" = "nft" ]; then
    nft list table inet lxd_block >/dev/null 2>&1 || nft add table inet lxd_block || exit 1
    nft flush chain inet lxd_block forward_block 2>/dev/null || true
    nft list chain inet lxd_block forward_block >/dev/null 2>&1 || \
        nft 'add chain inet lxd_block forward_block { type filter hook forward priority 0; policy accept; }' || exit 1
    for port in "${blocked_ports[@]}"; do
        nft add rule inet lxd_block forward_block oifname "eth0" tcp dport "$port" drop || exit 1
        nft add rule inet lxd_block forward_block oifname "eth0" udp dport "$port" drop || exit 1
    done
else
    for port in "${blocked_ports[@]}"; do
        iptables -C FORWARD -o eth0 -p tcp --dport "$port" -j DROP 2>/dev/null || \
        iptables -I FORWARD -o eth0 -p tcp --dport "$port" -j DROP || exit 1
        iptables -C FORWARD -o eth0 -p udp --dport "$port" -j DROP 2>/dev/null || \
        iptables -I FORWARD -o eth0 -p udp --dport "$port" -j DROP || exit 1
    done
fi
